...
#ifndef __AI_LSTM_H__
#define __AI_LSTM_H__
#pragma once

#include "ai_platform.h"

#define AI_LSTM_MODEL_NAME          "lstm"

#define AI_LSTM_IN_NUM       (1)
#define AI_LSTM_IN { \
  AI_BUFFER_OBJ_INIT(AI_BUFFER_FORMAT_FLOAT, 3, 1, 1, 1, NULL), \
}
#define AI_LSTM_IN_SIZE { \
  (3 * 1 * 1), \
}
#define AI_LSTM_IN_1_SIZE  (3 * 1 * 1)
#define AI_LSTM_IN_1_SIZE_BYTES  ((3 * 1 * 1) * 4)


#define AI_LSTM_OUT_NUM      (1)
#define AI_LSTM_OUT { \
  AI_BUFFER_OBJ_INIT(AI_BUFFER_FORMAT_FLOAT, 1, 1, 1, 1, NULL), \
}
#define AI_LSTM_OUT_SIZE { \
  (1 * 1 * 1), \
}
#define AI_LSTM_OUT_1_SIZE  (1 * 1 * 1)
#define AI_LSTM_OUT_1_SIZE_BYTES  ((1 * 1 * 1) * 4)
 ...
 * @ref ai_lstm_create. Both the activations memory buffer 
 ...
 * header generated by the code generation tool. See AI_LSTM_*
 * defines into file @ref lstm.h for all network sizes defines
 *
 * @param network an opaque handle to the network context
